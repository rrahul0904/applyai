...
export function EmployerWorkspace() {
  ...
  const applicantRows: Array<Record<string, unknown>> = Array.isArray(applicants.data) ? applicants.data : [];
  ...
  return <>
    ...
    {!org ? <Card className="detail-section"><EmptyState icon={<Building2/>} title="Create an employer organization" description="An organization is the verified boundary for recruiters, jobs and applicants."/><form className="form-stack" onSubmit={(event) => { event.preventDefault(); if (orgName.trim()) createOrg.mutate(); }}><Field label="Organization name" htmlFor="org-name"><Input id="org-name" value={orgName} onChange={(e) => setOrgName(e.target.value)} /></Field><Button type="submit" disabled={!orgName.trim()}><Plus size={16}/>Create organization</Button></form></Card> : <>
      ...
      {applicantJobId ? <Card id="applicants" className="detail-section"><div className="section-header"><div><h2>Applicant pipeline</h2><p>First-party ApplyAI submissions appear here immediately.</p></div><UsersRound/></div><div className="list-stack">{applicantRows.map((row) => <div className="note" key={String(row.id)}><strong>{String(row.candidate_email)}</strong><span>{String(row.stage)} · {String(row.application_status)}</span></div>)}{applicantRows.length === 0 ? <p className="muted">No applicants yet.</p> : null}</div></Card> : null}
    </>}
  </>;
}
