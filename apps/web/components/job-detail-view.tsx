...
import { Bookmark, BriefcaseBusiness, Building2, CalendarDays, MapPin, WalletCards } from "lucide-react";
...
export function JobDetailView({ jobId }: { jobId: string }) {
  ...
  const applying = useMutation({
    ...
    onSuccess: (application) => { queryClient.invalidateQueries({ queryKey: ["applications"] }); router.push(`/applications/${application.id}`); },
  ...
  return <div className="detail-grid">
    <div className="detail-main">
      <Card className="detail-hero">
        <div className="detail-title-row">
          ...
          <div><p className="eyebrow">{item.data_origin === "DEVELOPMENT_SEED" ? "Development data" : "Job opportunity"}</p><h1>{item.title}</h1><p className="job-company">{item.company_name}</p><div className="job-meta">
      ...
      {item.requirements.length ? <Card className="detail-section"><h2>Requirements</h2><ul>{item.requirements.map((requirement) => <li key={requirement}>{requirement}</li>)}</ul></Card> : null}
    ...
    <aside className="detail-aside"><Card className="sticky-actions">
      <Button onClick={() => applying.mutate()} disabled={applying.isPending}>Track application</Button>
      <Button variant="secondary" onClick={() => saving.mutate()} disabled={saving.isPending} aria-pressed={item.saved}><Bookmark size={17} fill={item.saved ? "currentColor" : "none"} />{item.saved ? "Saved" : "Save job"}</Button>
      <div className="facts-list"><div className="fact-row"><Building2 size={17} /><div><strong>Company</strong><span>{item.company_name}</span></div></div><div className="fact-row"><CalendarDays size={17} /><div><strong>Posted</strong><span>{formatDate(item.posted_at)}</span></div></div><div className="fact-row"><CalendarDays size={17} /><div><strong>Last verified</strong><span>{formatDate(item.last_seen_at)}</span></div></div></div>
      {item.source_url ? <a className="ui-button ui-button-ghost ui-button-small" href={item.source_url} target="_blank" rel="noreferrer">Open source listing</a> : null}
    </Card></aside>
  </div>;
}
