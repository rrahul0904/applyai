...
export function JobImportWorkspace() {
  const params = useSearchParams(); const [url, setUrl] = useState(params.get("url") ?? ""); const [importId, setImportId] = useState<string | null>(null);
  ...
  useEffect(() => { const incoming = params.get("url"); if (incoming) setUrl(incoming); }, [params]);
