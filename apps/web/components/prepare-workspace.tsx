...
import { prepareApi, type Course, type InterviewPack, type LearningPath, type MockInterview } from "@/lib/api/prepare-client";
  ...
  return (
    <>
      <div className="dashboard-grid">
        <Card><p className="eyebrow">Career readiness</p><h2>{data.overall_score}%</h2><Badge tone={scoreTone(data.overall_score)}>{data.band}</Badge></Card>
        {Object.entries(data.breakdown).map(([key, value]) => <Card key={key}><p className="eyebrow">{key}</p><h2>{value.score}%</h2><p className="muted">{value.weight}% of readiness</p></Card>)}
      </div>
      <Card className="detail-section">
        <h2>Next best actions</h2>
        <div className="list-stack">{data.next_actions.map((item) => <div key={item} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}><CheckCircle2 size={17} style={{ marginTop: 3 }} /><span>{item}</span></div>)}</div>
      </Card>
    </>
  );
}
  ...
  return (
    <Card className="detail-section">
      <div className="section-header">
        <div><h2>Skill intelligence</h2><p>Job requirements compared only against verified profile and learning evidence.</p></div>
        <Button variant="secondary" onClick={() => analyze.mutate()} disabled={analyze.isPending}><RefreshCw size={15} />Re-analyze</Button>
      </div>
      <div className="list-stack">
        {(gaps.data?.items ?? []).map((gap) => <div key={gap.id} style={{ borderTop: "1px solid var(--border)", paddingTop: 14 }}>
        ...
        {!gaps.data?.items.length ? <EmptyState title="No extracted skill requirements" description="This job does not yet have normalized skill requirements to compare." /> : null}
      </div>
    </Card>
  );
}
...
function CourseViewer({ courseId }: { courseId: string }) {
  ...
  useEffect(() => { if (!selectedLessonId && lessons[0]) setSelectedLessonId(lessons[0].id); }, [lessons, selectedLessonId]);
  const complete = useMutation({
    mutationFn: (mastery: number | undefined) => lesson ? prepareApi.setLessonProgress(lesson.id, true, mastery) : Promise.resolve(null),
    onSuccess: async () => { await queryClient.invalidateQueries({ queryKey: ["prepare-course", courseId] }); toast.success("Lesson progress saved"); },
  });
  ...
  return (
    <div className="detail-grid">
      <div className="detail-main">
        <Card className="detail-section">
          <div className="section-header"><div><p className="eyebrow">{course.data.skill}</p><h2>{lesson.title}</h2><p>{lesson.summary}</p></div><Button size="small" variant="secondary" onClick={() => setBionic((value) => !value)}>{bionic ? "Standard reading" : "Bionic reading"}</Button></div>
          <BionicText text={lesson.content_markdown} enabled={bionic} />
          <div className="button-row" style={{ marginTop: 18 }}><Button onClick={() => complete.mutate(undefined)} disabled={lesson.completed || complete.isPending}>{lesson.completed ? "Completed" : "Mark complete"}</Button>{lesson.mastery_score !== null ? <Badge tone="success">Mastery {lesson.mastery_score}%</Badge> : null}</div>
        </Card>
        <Card className="detail-section">
          <h2><BrainCircuit size={18} style={{ verticalAlign: "middle", marginRight: 8 }} />Lesson tutor</h2>
          <Field label="Ask about this lesson" htmlFor="lesson-tutor"><Textarea id="lesson-tutor" rows={4} value={tutorQuestion} onChange={(event) => setTutorQuestion(event.target.value)} /></Field>
          <Button onClick={() => tutor.mutate()} disabled={!tutorQuestion.trim() || tutor.isPending}>Ask tutor</Button>
          {tutorAnswer ? <div style={{ marginTop: 16, whiteSpace: "pre-wrap" }}>{tutorAnswer}</div> : null}
        </Card>
...
function LearningPanel({ jobId }: { jobId: string }) {
  ...
  const create = useMutation({ mutationFn: () => prepareApi.createLearningPath(jobId), onSuccess: (data) => { setPath(data); if (data.courses[0]) setCourseId(data.courses[0].id); } });
...
function MockInterviewPanel({ jobId }: { jobId: string }) {
  ...
  const speakQuestion = useCallback(() => {
    if (!mock?.current_question || typeof window === "undefined" || !("speechSynthesis" in window)) return;
    window.speechSynthesis.cancel();
    window.speechSynthesis.speak(new SpeechSynthesisUtterance(mock.current_question.question));
  }, [mock]);

  useEffect(() => { if (channel !== "WRITTEN" && mock?.current_question) speakQuestion(); }, [channel, mock?.current_question?.turn_id, speakQuestion]);
  useEffect(() => () => { mediaStreamRef.current?.getTracks().forEach((track) => track.stop()); if (recordingUrl) URL.revokeObjectURL(recordingUrl); }, [recordingUrl]);
  ...
  async function uploadRecording() {
    ...
    if (intent.upload_mode === "DIRECT") {
      ...
      const body = new FormData(); body.append("file", recording, "mock-interview.webm");
  ...
  return <Card className="detail-section">
    ...
    {mock?.current_question ? <div className="list-stack">
      <div><p className="eyebrow">Question {mock.current_question.position}</p><h2>{mock.current_question.question}</h2>{channel !== "WRITTEN" ? <Button size="small" variant="ghost" onClick={speakQuestion}><Play size={14}/>Read aloud</Button> : null}</div>
      {channel !== "WRITTEN" ? <div className="button-row">{recorderRef.current?.state === "recording" ? <Button variant="secondary" onClick={stopRecording}><Square size={15}/>Stop recording</Button> : <Button variant="secondary" onClick={() => startRecording().catch((error: Error) => toast.error(error.message))}>{channel === "VIDEO" ? <Video size={15}/> : <Mic size={15}/>}Record answer</Button>}{recordingUrl ? channel === "VIDEO" ? <video src={recordingUrl} controls style={{ width: 300, maxWidth: "100%" }} /> : <audio src={recordingUrl} controls /> : null}</div> : null}
      <Field label={channel === "WRITTEN" ? "Your answer" : "Answer transcript (used for scoring)"} htmlFor="mock-answer"><Textarea id="mock-answer" rows={8} value={answer} onChange={(event) => setAnswer(event.target.value)} placeholder={channel === "WRITTEN" ? "Answer as you would in the interview…" : "Paste or type the spoken answer transcript here. Your audio/video is stored separately and privately."} /></Field>
...
export function PrepareWorkspace({ jobId }: { jobId: string }) {
  ...
  return <>
    ...
    <div className="button-row" style={{ marginBottom: 18 }}>
      {(["overview", "skills", "learn", "prep", "mock"] as const).map((item) => <Button key={item} size="small" variant={tab === item ? "secondary" : "ghost"} onClick={() => setTab(item)}>{item === "prep" ? "Interview prep" : item.charAt(0).toUpperCase() + item.slice(1)}</Button>)}
    </div>
