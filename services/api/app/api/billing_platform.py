...
def _subscription(session: Session, user: User) -> Subscription:
    ...
    if item is None:
        item = Subscription(
            user_id=user.id, plan="FREE", status="ACTIVE", provider="INTERNAL", usage={}
        )
        session.add(item)
        session.commit()
...
def get_subscription(
    ...
    return {
        ...
        "usage": item.usage,
        "entitlements": PLAN_ENTITLEMENTS.get(item.plan, PLAN_ENTITLEMENTS["FREE"]),
    }
...
def get_entitlements(
    ...
    return {
        "plan": item.plan,
        "status": item.status,
        **PLAN_ENTITLEMENTS.get(item.plan, PLAN_ENTITLEMENTS["FREE"]),
        "usage": item.usage,
    }
...
def create_checkout(
    payload: CheckoutWrite,
    user: User = Depends(get_current_user),
    session: Session = Depends(get_session),
) -> dict[str, Any]:
    if not get_settings().billing_enabled:
        raise HTTPException(
            status_code=503, detail="Paid plans are disabled for the zero-cost pilot"
        )
    ...
    try:
        with httpx.Client(timeout=20.0) as client:
            response = client.post(
                f"{stripe_api_base_url()}/v1/checkout/sessions",
...
def create_portal(
    ...
    secret = os.getenv("STRIPE_SECRET_KEY")
    if not secret:
        raise HTTPException(status_code=503, detail="Stripe configuration is incomplete")
    settings = get_settings()
    try:
        with httpx.Client(timeout=20.0) as client:
            response = client.post(
                f"{stripe_api_base_url()}/v1/billing_portal/sessions",
                headers={"Authorization": f"Bearer {secret}"},
                data={
                    "customer": item.provider_customer_id,
                    "return_url": f"{settings.web_origin}/settings",
                },
            )
        response.raise_for_status()
    except httpx.HTTPError as exc:
...
async def stripe_webhook(
    ...
    if billing_provider() != "stripe" or not secret:
    ...
    if event_type == "checkout.session.completed":
        sub.plan = str(metadata.get("plan") or "PRO")
        sub.status = "ACTIVE"
        sub.provider = "STRIPE"
        sub.provider_customer_id = obj.get("customer")
        sub.provider_subscription_id = obj.get("subscription")
    elif event_type in {"customer.subscription.updated", "customer.subscription.created"}:
        sub.plan = str(metadata.get("plan") or sub.plan)
        sub.status = str(obj.get("status") or "ACTIVE").upper()
        sub.provider = "STRIPE"
        ...
        if obj.get("current_period_end"):
            sub.current_period_end = datetime.fromtimestamp(
                int(obj["current_period_end"]), tz=timezone.utc
            )
    ...
    session.add(
        BillingLedgerEvent(
            ...
            metadata_json={"plan": sub.plan, "status": sub.status},
        )
    )
    session.commit()
    return {"accepted": True}
