...
def _source_record(
    ...
    registry_id = checkpoint.get("source_registry_id")
    registry = session.get(JobSourceRegistry, uuid.UUID(registry_id)) if registry_id else None
    return {
...
def build_demo(session: Session) -> dict[str, Any]:
    ...
    for provider in ("GREENHOUSE", "LEVER", "ASHBY"):
        counts = pipeline.run(
            registries[provider],
            DemoSourceConnector(
                JobSourceType(provider),
                DEMO_SOURCE_IDENTITIES[provider],
                records[provider],
            ),
        )
    ...
    for job, company in jobs:
        ...
        sources = [_source_record(session, link, source) for link, source in links]
        provenance = [
            {
    ...
    demo_source_ids = [item.id for item in registries.values()]
    invalid_source_ids = list(
        session.scalars(
            select(JobSource.id).where(
                JobSource.checkpoint["source_registry_id"].astext.in_(
                    [str(item) for item in demo_source_ids]
                )
            )
        )
    )
        ...
        list(
            session.scalars(
                select(RawJobPostingModel.id).where(
                    RawJobPostingModel.job_source_id.in_(invalid_source_ids),
...
def _provider_badge(provider: str) -> str:
    key = provider.lower()
    return f'<span class="provider provider-{key}">{html.escape(provider)}</span>'
...
def render_demo_html(report: dict[str, Any]) -> str:
    ...
    source_cards = "".join(
        (
            ...
            f'<div class="source-head">{_provider_badge(item["provider"])}'
            f'<span class="health">{html.escape(item["health_status"])}</span></div>'
            f'<h3>{html.escape(item["source_name"])}</h3>'
            f'<p>{html.escape(item["trust_level"].replace("_", " ").title())} · priority {item["priority"]}</p>'
            '<div class="mini-grid">'
            f'<span>Fetched <strong>{item["latest_run"]["fetched"]}</strong></span>'
            f'<span>Valid <strong>{item["latest_run"]["valid"]}</strong></span>'
            f'<span>Invalid <strong>{item["latest_run"]["invalid"]}</strong></span>'
            f'<span>Deduped <strong>{item["latest_run"]["deduplicated"]}</strong></span>'
            '</div></article>'
        )
    ...
    job_rows = "".join(
        (
            '<tr>'
            f'<td><strong>{html.escape(job["title"])}</strong><small>{html.escape(job["company"])}</small></td>'
            f'<td><span class="status status-{job["status"].lower()}">{html.escape(job["status"])}</span></td>'
            f'<td>{html.escape(job["location"])}<small>{html.escape(job["work_mode"])}</small></td>'
            f'<td>{_money(job["salary"]["minimum"])}–{_money(job["salary"]["maximum"])}</td>'
            f'<td>{job["source_count"]}</td><td>{job["version_count"]}</td></tr>'
        )
        for job in report["canonical_jobs"]
    )
    shared = report["shared_job"]
    shared_source_rows = "".join(
        (
            '<tr>'
            f'<td>{_provider_badge(source["provider"])}</td>'
            f'<td>{html.escape(source["trust_level"].replace("_", " ").title())}</td>'
            f'<td>{"Primary" if source["is_primary"] else "Linked"}</td>'
            f'<td>{html.escape(source["dedup_reason"].replace("_", " ").title())}</td>'
            f'<td>{source["miss_count"]}</td></tr>'
        )
    ...
    provenance_rows = "".join(
        (
            '<tr>'
            f'<td>{html.escape(item["field"].replace("_", " ").title())}</td>'
            f'<td>{html.escape(item["selection_reason"].replace("_", " "))}</td>'
            '</tr>'
        )
        for item in shared["provenance"]
    )
    checks = "".join(
        f'<li><span>✓</span>{html.escape(name.replace("_", " ").title())}</li>'
...
<p class="lede">Generated by the real ApplyAI registered-source pipeline: validation, raw persistence, canonicalization, deduplication, source authority, field provenance, and multi-source freshness.</p>
<nav class="tabs" aria-label="Demo views"><button class="tab active" data-tab="overview">Platform overview</button><button class="tab" data-tab="provenance">Dedup & provenance</button><button class="tab" data-tab="evidence">Execution evidence</button></nav>
<section id="overview" class="view active">
<div class="metric-grid"><div class="metric"><span>Providers</span><strong>{totals['provider_sources']}</strong></div><div class="metric"><span>Valid postings</span><strong>{totals['source_postings']}</strong></div><div class="metric"><span>Canonical jobs</span><strong>{totals['canonical_jobs']}</strong></div><div class="metric"><span>Deduplicated</span><strong>{totals['deduplicated_postings']}</strong></div><div class="metric"><span>Invalid retained</span><strong>{totals['invalid_quarantined']}</strong></div><div class="metric"><span>Shared-job sources</span><strong>{totals['shared_job_sources']}</strong></div></div>
<div class="source-grid">{source_cards}</div>
<div class="panel"><h2>Canonical candidate catalog</h2><p class="panel-sub">Six valid provider postings become four candidate-visible jobs.</p><table><thead><tr><th>Job</th><th>Status</th><th>Location</th><th>Compensation</th><th>Sources</th><th>Versions</th></tr></thead><tbody>{job_rows}</tbody></table></div>
<div class="panel"><h2>Data flow</h2><p class="panel-sub">Every step executes during artifact generation.</p><div class="flow">{flow}</div></div>
</section>
<section id="provenance" class="view">
<div class="panel"><h2>Shared canonical job: {html.escape(shared['title'])}</h2><p class="panel-sub">Three provider records converge on one active job. The highest-authority source owns candidate-facing fields.</p><table><thead><tr><th>Provider</th><th>Trust</th><th>Role</th><th>Dedup reason</th><th>Miss count</th></tr></thead><tbody>{shared_source_rows}</tbody></table></div>
<div class="split"><div class="panel"><h2>Field-level provenance</h2><p class="panel-sub">The selected source is recorded for each canonical field.</p><table><thead><tr><th>Field</th><th>Selection reason</th></tr></thead><tbody>{provenance_rows}</tbody></table></div><div class="panel"><h2>Freshness result</h2><p class="panel-sub">Lever omitted the shared posting on a later run.</p><div class="callout"><strong>Canonical status: {report['lifecycle_run']['canonical_job_status']}</strong><br/>The job stays active because Greenhouse and Ashby remain fresh.</div><div class="scope"><span>Lever miss count: 1</span><span>Greenhouse: primary</span><span>Ashby: linked</span></div></div></div>
</section>
<section id="evidence" class="view"><div class="split"><div class="panel"><h2>Acceptance checks</h2><p class="panel-sub">Artifact generation fails when any check is false.</p><ul class="checks">{checks}</ul></div><div class="panel"><h2>Demo boundaries</h2><p class="panel-sub">Executed logic is separated from intentionally excluded external services.</p><ul class="checks"><li><span>✓</span>Real SQLAlchemy models and PostgreSQL</li><li><span>✓</span>Real source ingestion and canonicalization</li><li><span>✓</span>Real dedup, authority, provenance, and freshness</li><li><span>✓</span>No AWS account required</li><li><span>✓</span>No live provider calls</li></ul></div></div><div class="panel"><h2>Machine-readable totals</h2><pre style="white-space:pre-wrap;color:#b9d2eb;background:#07111f;padding:18px;border-radius:12px">{html.escape(json.dumps(report['totals'], indent=2))}</pre></div></section>
...
<script>for(const button of document.querySelectorAll('.tab')){{button.addEventListener('click',()=>{{document.querySelectorAll('.tab').forEach(item=>item.classList.remove('active'));document.querySelectorAll('.view').forEach(item=>item.classList.remove('active'));button.classList.add('active');document.getElementById(button.dataset.tab).classList.add('active');}});}}</script>
</body>
</html>"""
