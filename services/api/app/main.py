...
from app.api import (
    ...
    interview_execution, interview_intelligence, interview_intelligence_catalog_admin,
    interview_intelligence_evidence, job_imports, jobs, me, onboarding, privacy,
    profiles, recruiter_lens, referrals, resume_shares, resumes, semantic_matching,
)
    ...
    try:
        with engine.connect() as connection:
            ...
            if settings.auth_provider == "supabase":
                operator_auth_configured = bool(connection.scalar(text("SELECT EXISTS (SELECT 1 FROM user_roles ur JOIN roles r ON r.id = ur.role_id WHERE r.name IN ('operator', 'admin'))")))
            else:
...
for router in (
    me.router,onboarding.router,profiles.router,resumes.router,jobs.router,applications.router,
    career_memory.router,career_intelligence_v2.router,candidate_platform.router,
    interview_intelligence.router,interview_execution.router,referrals.router,
    semantic_matching.router,company_intelligence.router,employer_platform.router,billing_platform.router,privacy.router,
):
    app.include_router(router,prefix="/api/v1")
for product_router in (candidate_workspace.router,career_product_contract.router,career_product_polish.router,career_product.router,career_system.router,recruiter_lens.router,resume_shares.router,agents.router,application_agent.router,application_agent_documents.router):
...
for internal_router in (
    internal_agents.router,application_agent.internal_router,application_agent_documents.internal_router,
    internal_job_sources.router,internal_job_discoveries.router,internal_job_quality.router,internal_job_supply.router,
    internal_ai_quality.router,internal_ai_evaluation.router,internal_operations.router,internal_platform_admin.router,
    interview_intelligence.internal_router,interview_intelligence_evidence.router,
    interview_intelligence_catalog_admin.router,referrals.internal_router,internal_worker.router,
):
