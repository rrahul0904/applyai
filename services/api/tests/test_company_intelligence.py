...
def test_company_intelligence_is_evidence_backed(client, database_url):
    ...
    with Session(engine) as session:
        ...
        job = Job(company_id=company.id, title="AI Platform Director", normalized_title="ai platform director", description="Lead our generative AI platform and distributed team.", search_document="ai platform director", status="ACTIVE", data_origin="DEVELOPMENT_SEED")
        session.add(job); session.flush()
        session.add(JobLocation(job_id=job.id, location_text="Boston, MA", work_mode="HYBRID"))
        session.add(JobSkill(job_id=job.id, name="Python"))
        session.add(JobRequirement(job_id=job.id, requirement_type="OTHER", text="Visa sponsorship available for qualified candidates"))
        session.commit(); job_id = str(job.id)
    engine.dispose()
