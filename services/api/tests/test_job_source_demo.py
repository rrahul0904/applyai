...
def test_job_source_demo_builds_real_pipeline_evidence(database_url, tmp_path):
    ...
    # Match the production SessionLocal contract. The demo must not depend on
    # SQLAlchemy's default autoflush behavior to expose provenance evidence.
    ...
    assert len(report["shared_job"]["provenance"]) == 7
    assert sum(
